...
export class NextRESTClient {
  private readonly _DELETE: (request: Request, args: { params: Promise<{ slug: string[] }> }) => Promise<Response>

  private readonly _GET: (request: Request, args: { params: Promise<{ slug: string[] }> }) => Promise<Response>

  private readonly _GRAPHQL_POST: (request: Request) => Promise<Response>

  private readonly _PATCH: (request: Request, args: { params: Promise<{ slug: string[] }> }) => Promise<Response>

  private readonly _POST: (request: Request, args: { params: Promise<{ slug: string[] }> }) => Promise<Response>

  private readonly _PUT: (request: Request, args: { params: Promise<{ slug: string[] }> }) => Promise<Response>
  ...
  async GET(path: ValidPath, options: Omit<RequestInit, 'body'> & RequestOptions = {}): Promise<Response> {
    const { slug, params, url } = this.generateRequestParts(path)
    const { query, ...rest } = options || {}
    const queryParams = generateQueryString(query, params)
  ...
  async POST(path: ValidPath, options: FileArg & RequestInit & RequestOptions = {}): Promise<Response> {
    const { slug, params, url } = this.generateRequestParts(path)
